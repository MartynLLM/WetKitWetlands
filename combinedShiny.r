...
water_chemistry_combined_app <- function(db_path = "water_samples.db") {
  ...
  server <- function(input, output, session) {
    ...
    all_waterbodies <- reactive({
      dbGetQuery(con, "SELECT DISTINCT waterbody, region FROM chem_to_display ORDER BY region, waterbody")
    })
    ...
    observe({
      ...
      # Use waterbody names directly as values, with region in parentheses
    ...
    get_parameter_data <- reactive({
      ...
      if (length(input$waterbody_param) > 0 && !("All" %in% input$waterbody_param)) {
        waterbodies <- paste0("'", paste(input$waterbody_param, collapse = "','"), "'")
        waterbody_filter <- paste0(" AND x_data.waterbody IN (", waterbodies, ")")
      }
    ...
    get_paired_data <- reactive({
      ...
      if (length(input$waterbody_io) > 0 && !("All" %in% input$waterbody_io)) {
        waterbodies <- paste0("'", paste(input$waterbody_io, collapse = "','"), "'")
        waterbody_filter <- paste0(" AND inlet_data.waterbody IN (", waterbodies, ")")
      }
